...
namespace Inventory.UnitOfWork
{
    ...
    {
        Task<TEntity?> GetByIdAsync(int? id); // Asynchronous method to get an entity by id
        ...
        Task<TEntity?> GetIncluding(Expression<Func<TEntity, bool>> predicate, params Expression<Func<TEntity, object>>[] includeProperties);
        TEntity Get(int id);
